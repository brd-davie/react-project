          ...
          <div className='w-full pt-0 md:p-5 md:pt-0'>
            <div className="flex flex-row-reverse">
              <div className='flex flex-col w-[60%] lg:w-full'>
                ...
                <h4 className='text-lg md:text-xl text-white mb-3 opacity-[.6] md:my-3 md:mt-0 pr-2'>{character.name_kanji}</h4>
                <div className='flex flex-col'>
                  ...
                  <ul className='flex flex-wrap gap-2 text-white opacity-[.3] mb-2'>
                    {character.nicknames.map((names, index) => (
                      <li key={index} className='text-lg'>{names},</li>
                    ))}
                  </ul>
                </div>
                <p className='text-white opacity-[.3] text-lg mb-4'>Favorites: <span className=''>{character.favorites}</span></p>
              </div>
              <div className='w-[40%] md:w-[50%] flex pr-3'>
                <img src={character.images.jpg.image_url} alt="Avatar" className='w-full h-full rounded-lg aspect-[2/3] object-cover' />
              </div>
            </div>
            ...
            <div className='carousel gap-3'>
              {character.anime.map((anime, index) => (
                <Link to={`/anime/anime-list/${anime.anime.mal_id}/anime-details`} key={index} className='carousel-item w-[100px] flex-col md:w-md max-w-xs'>
                  <img src={anime.anime.images.jpg.image_url} alt="" className='w-full object-cover rounded-md min-h-12 aspect-[2/3]' />
            ...
            <div className='carousel gap-3'>
              {character.manga.map((anime, index) => (
                <Link to={`/anime/anime-list/${anime.manga.mal_id}/anime-details`} key={index} className='carousel-item w-[100px] flex-col md:w-md max-w-xs'>
                  <img src={anime.manga.images.jpg.image_url} alt="" className='w-full object-cover h-[82%] rounded-md min-h-12 aspect-[2/3]' />
                  <div className='h-auto'>
            ...
            <div className='carousel flex gap-3'>
              {character.voices.map((voices, index) => (
                <div key={index} className='carousel-item flex flex-col w-[100px]'>
                  <img src={voices.person.images.jpg.image_url} alt='' className='w-full h-full object-cover rounded-md min-h-12 aspect-[2/3]' />
