...
const AnimeNews = ({ id }) => {
  ...
  return (
    <div className='p-3 mt-4 lg:mt-20 max-w-[1250px] mx-auto'>
      ...
      {
        news ? (
          <div className='w-full grid grid-cols-2 md:grid-cols-3 gap-3 lg:carousel'>
            {news.map((anime, index) => (
              <div key={index} id='custom-news' className="card lg:w-[40%] card-side rounded-md glass shadow-2xl md:flex-nowrap shadow-xl lg:carousel-item">
                <figure id='custom-figure' className='w-[40%] rounded-md lg:rounded-[inherit]'><img src={anime.images.jpg.image_url} alt="Images" className='w-full h-full object-cover' /></figure>
                <div className="card-body w-[60%] px-2 md:px-[16x] p-2">
                  ...
                  <p className='text-white text-xs opacity-[.6] lg:text-lg leading-[22px]'>{anime.excerpt}</p>
